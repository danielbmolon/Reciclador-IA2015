...
public class JanelaMatriz extends JFrame {
	...
	public JanelaMatriz(int tamanho) {
		...
		if (tamanho == 10) {
			nAgentes = 2;
        	nLixos = 6;
        	posicaoLixeirasLs = new int [4]; //[linha - Ls][Coluna - Ls][[linha - Ls2][Coluna - Ls2]...
        	posicaoLixeirasLo = new int [4]; //[linha - Lo][Coluna - Lo][[linha - Lo2][Coluna - Lo2]...
		...
		if (tamanho == 20) {
        	...
        	posicaoLixeirasLs = new int [6]; //[linha - Ls][Coluna - Ls][[linha - Ls2][Coluna - Ls2]...
        	posicaoLixeirasLo = new int [6]; //[linha - Lo][Coluna - Lo][[linha - Lo2][Coluna - Lo2]...
        	...
        	lixeiras[1] = new Lixeira("3", "Lo", "3", "10", "0"); //seta lixeira
        	...
        	lixeiras[3] = new Lixeira("3", "Lo", "16", "3", "0"); //seta lixeira
        	
			valores[16][10] = "Ls";
			posicaoLixeirasLs[4] = 16;
        	posicaoLixeirasLs[5] = 10;
        	lixeiras[4] = new Lixeira("3", "Ls", "16", "10", "0"); //seta lixeira
        	
			valores[16][16] = "Lo";
			posicaoLixeirasLo[4] = 16;
        	posicaoLixeirasLo[5] = 16;
        	lixeiras[5] = new Lixeira("3", "Lo", "16", "16", "0"); //seta lixeira
		...
		if (tamanho == 30) {
        	...
        	posicaoLixeirasLs = new int [8]; //[linha - Ls][Coluna - Ls][[linha - Ls2][Coluna - Ls2]...
        	posicaoLixeirasLo = new int [8]; //[linha - Lo][Coluna - Lo][[linha - Lo2][Coluna - Lo2]...
        	...
        	lixeiras[1] = new Lixeira("3", "Lo", "3", "15", "0"); //seta lixeira
        	
			valores[3][23] = "Ls";
			posicaoLixeirasLs[2] = 3;
        	posicaoLixeirasLs[3] = 23;
        	lixeiras[2] = new Lixeira("3", "Ls", "3", "23", "0"); //seta lixeira
        	
			valores[16][6] = "Lo";
			posicaoLixeirasLo[2] = 16;
        	posicaoLixeirasLo[3] = 6;
        	lixeiras[3] = new Lixeira("3", "Lo", "16", "6", "0"); //seta lixeira
        	
			valores[16][23] = "Ls";
			posicaoLixeirasLs[4] = 16;
        	posicaoLixeirasLs[5] = 23;
        	lixeiras[4] = new Lixeira("3", "Ls", "16", "23", "0"); //seta lixeira
        	
			valores[27][6] = "Lo";
			posicaoLixeirasLs[4] = 27;
        	posicaoLixeirasLs[5] = 6;
        	lixeiras[5] = new Lixeira("3", "Lo", "27", "6", "0"); //seta lixeira
        	
        	
			valores[27][15] = "Ls";
			posicaoLixeirasLs[6] = 27;
        	posicaoLixeirasLs[7] = 15;
        	lixeiras[6] = new Lixeira("3", "Ls", "27", "15", "0"); //seta lixeira
        	
			valores[27][23] = "Lo";
			posicaoLixeirasLs[6] = 27;
        	posicaoLixeirasLs[7] = 23;
        	lixeiras[7] = new Lixeira("3", "Lo", "27", "23", "0"); //seta lixeira
	...
	public void reciclar() { 
		//Reciclador  = 0 -- primeiro Reciclador, durante o while que seta Recicladores conforme valor de c
		//no final do laço, testa se c não é maior que o numero de Recicladores, caso seja, zera c e começa as ações novamente.
		int c = 0; 
		...
		if (posicao[0] != r1[c].getLinha()) {
			if (posicao[0] < r1[c].getLinha()) { //se linha a ir menor que linha atual -> subir
				for (int li = r1[c].getLinha() - 1; li >= posicao[0]; li--) { //anda atá a posição com lixo
					
					andar(c,li,r1[c].getColuna());

					
					System.out.println(posicao[0] + "-" + posicao[1] + " || " + r1[c].getLinha() + " - "+ r1[c].getColuna());
				}
			}else if (posicao[0] > r1[c].getLinha()) { //se linha a ir maior que linha atual -> descer
			...
			if (posicao[1] < r1[c].getColuna()) { //esquerda
				for (int cl = r1[c].getColuna() - 1; posicao[1] <= cl; cl--) {
					...
					if (posicao[1] > r1[c].getColuna()) { //direita
			...
			System.out.println(posicao[0] + "-" + posicao[1] + " || " + r1[c].getLinha() + " - "+ r1[c].getLinha() + " - ELSE");
		}
	...
	public void andar(int reciclador,int linha, int coluna){
		...
		if(valorCelula == "o"){
			if(!r1[reciclador].sacoLixoCheio("o")){ //inverte o retorno (!) para entrar no if caso nao cheio
				r1[reciclador].setSacoLixo("o");
				
				if(r1[reciclador].sacoLixoCheio("o")){ //testa novamente para saber se tem que caminha ate a lixeira
					System.out.println("Euuu" + r1[reciclador].getSacoLixo("o"));//apagar
		...
		if(valorCelula == "s"){
			if(!r1[reciclador].sacoLixoCheio("s")){ //inverte o retorno (!) para entrar no if caso nao cheio
				...
				if(r1[reciclador].sacoLixoCheio("s")){ //testa novamente para saber se tem que caminha ate a lixeira
					System.out.println("Euuu" + r1[reciclador].getSacoLixo("s"));//apagar
	...
	public class Start implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			AgenteReciclador agReciclador = new AgenteReciclador(); //comeca a reciclar
			agReciclador.start();
